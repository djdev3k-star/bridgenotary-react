      ...
      <section className="relative overflow-hidden w-screen left-1/2 -translate-x-1/2 bg-white border-b border-neutral-200">
        <div className="relative max-w-6xl mx-auto px-6 py-20 md:py-24">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link to={bookingLink} className="bg-electric-blue text-white font-semibold rounded px-8 py-4 text-lg shadow-lg hover:shadow-xl hover:bg-electric-blue/90 transition-all">
              {ctaText}
            </Link>
        ...
        <section className="py-20 md:py-24 bg-white border-t border-neutral-200">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-16">
              <p className="text-sm font-semibold tracking-[0.2em] uppercase text-electric-blue mb-3">Why Choose Us</p>
              <h2 className="text-3xl md:text-4xl font-bold text-proof mb-4">What Sets Bridge Notary Apart</h2>
              <p className="text-lg text-neutral-600 max-w-3xl mx-auto">Experience, precision, and reliability—delivered by NNA-certified signing agents who understand your transaction.</p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {highlights.map((item, index) => (
                <div key={item.title} className="relative">
                  <div className="absolute -top-4 -left-2 w-10 h-10 bg-electric-blue/10 flex items-center justify-center text-electric-blue font-bold text-lg">
                    {index + 1}
                  </div>
                  <div className="pt-8 pl-4">
                    <h3 className="font-bold text-proof text-lg mb-3">{item.title}</h3>
                    <p className="text-neutral-600 leading-relaxed">{item.description}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-16 p-8 bg-electric-blue/5 border border-electric-blue/20">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                <div className="flex-shrink-0">
                  <svg className="w-12 h-12 text-electric-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                </div>
                <div>
                  <h4 className="font-bold text-proof text-lg mb-1">Independent & Impartial</h4>
                  <p className="text-neutral-600">As a commissioned notary, we serve as an impartial witness to the signing—verifying signer identity, ensuring documents are signed voluntarily, and providing legally required notarization. We do not provide legal advice or represent any party in the transaction.</p>
                </div>
              </div>
            </div>
          </div>
